...
describe('Home - Internacionalização', () => {
  ...
  it('deve permitir mudança de idioma se disponível', () => {
    ...
    cy.get('body').then(($body) => {
      if ($body.find('[data-cy="language-selector"], .language-selector, select[name="language"]').length > 0) {
        // Se existe seletor de idioma, testa a mudança
        cy.get('[data-cy="language-selector"], .language-selector, select[name="language"]').first().click()
