...
import { Container, Row, Col, Card, Form, Button, Tab, Tabs, Table } from 'react-bootstrap';
              ...
              <Form onSubmit={handleSubmit(onSubmit)} className="gids-form">
                ...
                <Card className="mb-4">
                  <Card.Body>
                    ...
                    <div className="gids-symptoms-section gids-basic-symptoms">
                      {[
                        'bowelSounds', 'vomiting', 'grv', 'ileus', 'distension',
                        'mildDiarrhea', 'bleedingNo', 'iap_12_20', 'noOral'
                      ].map((symptom) => (
                        <Form.Check
                          key={symptom}
                          type="checkbox"
                ...
                <Card className="mb-4">
                  <Card.Body>
                    ...
                    <div className="gids-symptoms-section gids-severe-symptoms">
                      {[
                        'severeDiarrhea', 'bleedingYes', 'prokinetics', 'iap_gt_20'
                      ].map((symptom) => (
                        <Form.Check
                          key={symptom}
                          type="checkbox"
                ...
                <Card className="mb-4">
                  <Card.Body className="text-center">
                    <div className={`gids-score-display ${getScoreClass(currentScore)}`}>
                      <h4 className="mb-0">
                        {t('gids.score')}: {currentScore} - {currentClassification}
                      </h4>
                    </div>
                    ...
                    {(() => {
                      ...
                      const lifeThreateningCount = details.lifeThreateningCount;
                      ...
                      if (lifeThreateningCount > 0) {
                        calculationReason = t('gids.debug.gids4', { count: lifeThreateningCount });
                      } else if (failureCount >= 3) {
                        calculationReason = t('gids.debug.gids3', { count: failureCount });
                      } else if (dysfunctionCount >= 1) {
                        calculationReason = t('gids.debug.gids2Dysfunction', { count: dysfunctionCount });
                      } else if (basicCount >= 3) {
                        calculationReason = t('gids.debug.gids2Basic', { count: basicCount });
                      } else if (basicCount >= 2) {
                        calculationReason = t('gids.debug.gids1Basic', { count: basicCount });
                      } else if (watchedValues.noOral && basicCount === 1) {
                        calculationReason = t('gids.debug.gids1NoOral');
                      } else {
                        calculationReason = t('gids.debug.gids0', { count: basicCount });
                      }
                      
                      return (
                        <div className="mt-3 text-start">
                          <small className="text-muted">
                            <div><strong>{t('gids.debug.selectedSymptoms')}</strong></div>
                            {basicCount > 0 && (
                              <div>• {t('gids.debug.basicSymptoms')} {basicCount}</div>
                            )}
                            {dysfunctionCount > 0 && (
                              <div>• {t('gids.debug.dysfunctionSymptoms')} {dysfunctionCount}</div>
                            )}
                            {failureCount > dysfunctionCount && (
                              <div>• {t('gids.debug.failureTotal')} {failureCount}</div>
                            )}
                            {lifeThreateningCount > 0 && (
                              <div>• {t('gids.debug.lifeThreatening')} {lifeThreateningCount}</div>
                            )}
                            <div className="mt-2 p-2 bg-light rounded">
                              <strong>{t('gids.debug.calculation')}</strong> {calculationReason}
                            </div>
                          </small>
                        </div>
                      );
                ...
                <div className="d-flex justify-content-between">
                  ...
                  <Button variant="primary" type="submit">
                    {savedData ? t('gids.measurementSaved') : t('gids.saveMeasurement')}
                  </Button>
                </div>
              </Form>
            </Tab>
            ...
            <Tab eventKey="summary" title={t('gids.summary')}>
              <Card className="mb-4">
                <Card.Body>
                  ...
                  <div>
                    <strong>{t('gids.patientName')}:</strong> {savedData?.patientName || '--'}<br />
                    <strong>{t('gids.patientId')}:</strong> {savedData?.patientId || '--'}<br />
                    <strong>{t('gids.patientAge')}:</strong> {savedData?.patientAge || '--'}<br />
                    <strong>{t('gids.measurementDate')}:</strong> {savedData?.admissionDate || '--'}
                  </div>
                </Card.Body>
              </Card>
              ...
              <Card className="mb-4">
                <Card.Body>
                  ...
                  <div>
                    {savedData ? (
                      <>
                        <p><strong>{t('gids.analysis.title')}</strong></p>
                        <p>{t('gids.analysis.scoreDisplay', { score: savedData.score, classification: savedData.classification })}</p>
                        {savedData.score >= 2 ? (
                          <p>{t('gids.analysis.highRiskRecommendation', { score: savedData.score, classification: savedData.classification })}</p>
                        ) : (
                          <p>{t('gids.analysis.lowRiskRecommendation', { score: savedData.score, classification: savedData.classification })}</p>
                        )}
                      </>
                    ) : (
              ...
              <div className="text-center">
                <Button variant="primary" onClick={exportReport} disabled={!savedData}>
                  {t('gids.exportReport')}
                </Button>
              </div>
            </Tab>
