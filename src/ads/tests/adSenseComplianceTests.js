...
const runComplianceTests = () => {
	...
	if (mainContent) {
		...
		console.log(
			contentLength >= 500 ? '✅' : '❌',
			`Mínimo 500 chars: ${contentLength >= 500}`,
		)
	} else {
	...
	const modals = document.querySelectorAll(
		'.modal.show, .overlay:not([style*="none"])',
	)
	console.log(
		modals.length === 0 ? '✅' : '❌',
	...
	console.log(
		finalValidation.isValid
			? '🟢 PÁGINA EM COMPLIANCE'
			: '🔴 PÁGINA COM VIOLAÇÕES',
	)
...
const simulateViolations = () => {
	...
	setTimeout(() => {
		...
		console.log(
			validation1.isValid ? '❌ Falhou' : '✅ Detectou',
			'violação de skeleton',
		)
		...
		setTimeout(() => {
			...
			console.log(
				validation2.isValid ? '❌ Falhou' : '✅ Detectou',
				'violação de data-no-ads',
			)
