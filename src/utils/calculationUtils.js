...
const mergeResults = (f1, f2, nonNut, proteinModule = 0, otherModule = 0) => {
	const totalCalories =
		f1.calories + f2.calories + nonNut.totalCalories + otherModule
	const totalCarbs = f1.carbs + f2.carbs + nonNut.macros.carbs
	const totalLipids = f1.lipids + f2.lipids + nonNut.macros.lipids
	const totalProtein =
		f1.protein + f2.protein + nonNut.macros.protein + proteinModule
	...
	return {
		...
		lipidsPercentage:
			totalCalories > 0 ? (lipidsCalories / totalCalories) * 100 : 0,
		proteinPercentage:
...
export const calculateResults = (
	...
	const infusionTime = patientData.infusionTime
		? parseFloat(patientData.infusionTime)
		: 0
	const proteinModule = patientData.proteinModule
		? parseFloat(patientData.proteinModule)
		: 0
	const otherModule = patientData.otherModule
		? parseFloat(patientData.otherModule)
		: 0
	...
	const nonNutritionalResults =
		calculateNonNutritionalCalories(nonNutritionalCals)
